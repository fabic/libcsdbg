#ifndef _CSDBG_PLUGIN
#define _CSDBG_PLUGIN 1

/**
	@file include/plugin.hpp

	@brief Class csdbg::plugin definition
*/
...
/**
	...
	register them with libcsdbg to be run upon function call and return. A plugin
	can be created by passing it the addresses of the instrumentation functions or
	by loading a plugin <b>DSO</b> module that implements and exports these two
	functions. A plugin invokes the system dynamic linker to find, load and link
	the module, so it must reside in one of the linker search directories.
	...
	libcsdbg functions) must <b>NOT</b> be instrumented by libcsdbg, as this will
	result in a recurse and a stack overflow

	@see tracer::register_plugin, tracer::unregister_plugin
	@see <a href="index.html#sec5_6"><b>5.6 Using the instrumentation plugin API</b></a>
*/
...
protected:
	...
	void *m_handle;						/**< @brief DSO handle (as provided by dlopen) */
