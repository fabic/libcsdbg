...
/**
	...
	@brief Class csdbg::plugin method implementation
*/
 ...
 *
 * @param[in] scope the full scope of the callbacks (namespace and/or class)
 *
 * @throws std::bad_alloc
 * @throws csdbg::exception
 *
 * @note If scope is NULL, the <b>C ABI</b> is used to resolve the symbols
 */
 ...
 *
 * @param[in] scope the full scope of the symbol (namespace and/or class)
 *
 ...
 *	in order to be callable from a plugin object. If scope is NULL, the C ABI is
 *	used to resolve the symbol
 */
modsym_t plugin::resolve(const i8 *nm, const i8 *scope) const
{
	...
	/* Mangle the symbol (g++ ABI mangling, Itanium IA64 compatible) */
	else {
		string tmp(scope);
		...
		try {
			...
			for (u32 i = 0, sz = parts->size(); likely(i < sz); i++) {
				string *token = parts->at(i);
				mangled->append("%d%s", token->length(), token->cstr());
			}
	...
	dlerror();
	void *sym = dlsym(m_handle, mangled->cstr());
